...
namespace SocialNetwork.DAL.Repositories {
    public class MessageRepository : BaseRepository, IMessageRepository {
        public int Create (MessageEntity messageEntity) {
            return Execute(@"insert into messages(content, sender_id, recipirnt_id) 
                             values(:content,:sender_id,:recipirnt_id)", messageEntity);
        }
        ...
        public IEnumerable<MessageEntity> FindByRecipientId (int recipientId) {
            return Query<MessageEntity>("select * from messages where recipirnt_id = :recipirnt_id", new { recipirnt_id = recipientId });
        }
